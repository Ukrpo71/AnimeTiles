...
public class ImageInventorySO : ScriptableObject
{
    ...
    public UnlockableImageInventoryData GetNextImageToShow()
    {
        if (Images.Where(n => n.Progress > 0 && n.Progress < 1).Count() > 0)
            return Images.FirstOrDefault(n => n.Progress > 0 && n.Progress < 1);
        else
        {
            return Images.FirstOrDefault(n => n.Progress == 1);
        }
    }
